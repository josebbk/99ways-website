...
- **Nav dropdown behavior — corrected spec (replaces the previous version of this rule; the delay-based timing below was tried and was unreliable, so it's been simplified):**
  ...
  - **Desktop — open/close timing (simplified, no delay):** treat the trigger label and its open dropdown menu as one combined hover region (e.g. a single wrapping element with `mouseenter`/`mouseleave`, not two separate listeners on trigger and menu that have to coordinate). Opens on `mouseenter` of that region. Closes **immediately** on `mouseleave` of that region — no timer, no grace period. This applies whether the cursor leaves toward empty space or moves directly to a different top-level trigger; either way, the previously open dropdown must close the instant the cursor exits its region.
  - **Desktop — "Optimization Experiences" sub-dropdown indicator:** a right-facing arrow (`>`), font and color matching the rest of the dropdown menu items (`--text-label`, same color) — not a generic icon glyph, it should read as part of the menu text. **Right-aligned to the far edge of the row** — the row needs `display: flex; justify-content: space-between` (or equivalent), not the arrow simply placed after the text with normal inline flow, which lets it drift left of the row's actual right edge. Purely a visual indicator on desktop — the actual reveal is still hover-triggered (see below), the arrow doesn't add its own separate interaction on desktop.
  - **Mobile/tablet:** click/tap-triggered, accordion behavior — opening one top-level dropdown closes any other open one immediately. Opening "Resources" reveals only its direct children (Guides and How-tos, PostHog Feature Breakdowns, Tools Comparison, Optimization Experiences) — nothing nested a level deeper than that.
  - **Mobile/tablet — "Optimization Experiences" split control (this has failed twice already — implement precisely as described, not approximately):** the row shows a down-facing arrow (▾) instead of the desktop `>`. This row must render as **two separate DOM elements with two separate click handlers**, each calling `stopPropagation()` so one never triggers the other:
    - The **arrow element** (its own `<button>` or clickable span, not a `::before`/`::after` pseudo-element): its handler toggles a dedicated `isFeaturedExperimentsOpen`-type state, and does nothing else — no navigation.
    - The **text label element**: its handler navigates to `/category/optimization-experience/`, and does nothing else — it must never read or toggle the "Featured Experiments" visibility state.
    - "Featured Experiments" visibility must be driven **only** by the arrow's own state — not derived from, or coupled to, whether "Resources" itself is open. Opening "Resources" must never cause "Featured Experiments" to appear as a side effect.
  - **"Featured Experiments" sub-item:** hidden by default in all cases. Desktop: reveals on hover over "Optimization Experiences." Mobile/tablet: reveals only via the arrow tap described above, independent of "Resources"' own open state.
