...
- **Dark, minimal, precise** — not corporate-generic, not decorative. Flat components, hairline borders, restrained color use (green reserved for CTA buttons).
...
- **One clear action**: "Book Intro Call." It appears twice (hero, pre-footer). A second green CTA ("Contact Us") exists in the header — same green fill, text color, and hover/press animation, but with an 8px border-radius as the sole visual differentiator. Both buttons use `--color-action` green.
...
- **Resolved:** Header "Contact Us" button restyled as CTA (green fill, hover glow/press). Changed from `btn-ghost` (transparent bg, border-only) to `btn-primary` class with 8px border-radius. Revises the `--color-action` "CTA-only" rule: the button is now a second sanctioned use of `--color-action` green alongside "Book Intro Call". Full spec in `ARCHITECTURE.md` §3/§4.
