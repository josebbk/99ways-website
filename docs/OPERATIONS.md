...
- **Mobile-first responsive check, not desktop-first.** Test at real mobile widths (not just a narrowed desktop browser) — 375px and 414px are good baselines — before checking desktop. Confirm zero horizontal page scroll anywhere; this has been a recurring bug specifically with the testimonial carousel.
...
- **Cropping/aspect-ratio checks against real images, not assumptions.** The testimonial carousel's "zero horizontal crop" rule needs verification against the actual 9 images' native dimensions, not just the code review — some images are more likely than others to reveal a cropping bug depending on their real aspect ratio.
